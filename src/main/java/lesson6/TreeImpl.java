...
public class TreeImpl<E extends Comparable<E>> implements Tree<E> {
    ...
    public Boolean isBalanced(int maxDepth){
        return maxDepth - getMinDepth() <= 1;
    }
    ...
    private int getNextDepth(Node<E> current, int depth) {
        if (current.getLeftChild() == null || current.getRightChild() == null) return depth;
        else {
            depth++;
            return Math.min(getNextDepth(current.getLeftChild(), depth), getNextDepth(current.getRightChild(), depth));
        }
    }
    ...
    private NodeAndParent doFind(E value) {
        ...
        @Override
        public int add(E value, int maxDepth) {
            int level = 1;
